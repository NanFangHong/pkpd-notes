...
6. pdf2htmlEX 每次只转换一页，避免 full 版本裁剪累计误差。
7. 每个 pdf2htmlEX 页面片段都在末尾追加本地覆盖样式，解除 `.pf/.pc` 的右下裁剪，并给外层 HTML 容器保留少量安全余量。
8. full HTML 用 iframe 逐页拼接，并用 `postMessage` 转发 pdf2htmlEX 的目录跳转。
9. 首页层级仿 dec41：笔记名对应两个入口，`HTML` 和 `PDF`；HTML 入口内部再放 full 和章节切片。
10. 目录标题中的轻量 LaTeX 在构建时渲染，避免依赖运行时 CDN。
11. GitHub Pages 发布已提交的 `site/`，不在远端重新编译 LaTeX/pdf2htmlEX。
