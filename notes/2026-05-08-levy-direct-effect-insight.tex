...
% Manually drawn fraction bars stay in the same pdf2htmlEX layer as the color blocks.
\draw (275.5,63.75) -- (292,63.75);
\draw (253.5,152.75) -- (270,152.75);
\draw (46.5,86.75) -- (63,86.75);
\draw (88.5,86.75) -- (105,86.75);
\draw (119.5,86.75) -- (136,86.75);
...
% Text Node
\draw (237.5,41.9) node [anchor=north west][inner sep=0.75pt]    {$m=$};
% Text Node
\draw (221.5,131.9) node [anchor=north west][inner sep=0.75pt]    {$k=$};
% Text Node
\draw (68.5,64.9) node [anchor=north west][inner sep=0.75pt]    {$=$};
\draw (108.5,64.9) node [anchor=north west][inner sep=0.75pt]    {$\cdot$};
% Text Node
...
Levy 的想法也自然导向一个经典判断：很多 drug effects 的 duration 主要由 plasma concentration 保持在某个 minimum effective concentration 之上的时间决定。这个阈值可以是 \(\textit{EC}_{10}\)、\(\textit{IC}_{10}\)，或者更一般的 \(\textit{MEC}\)。
...
\[
  T_{\mathrm{effect}}
  \approx
  \frac{1}{k}\log\!\left(\frac{C_0}{\textit{MEC}}\right).
\]
...
一个常见形式是
\[
  E(C)=E_0+\frac{\Emax C^\gamma}{\textit{EC}_{50}^{\gamma}+C^\gamma}.
\]
