...
{\small
  ...
  The intended reader has not taken 6.008/6.437 and has not used NONMEM. We begin with probability laws, conditioning, Bayes rule, and latent variables, then move gradually into \(\textit{PK}\) ODEs, individual parameters, population distributions, marginal likelihood, SAEM, and NONMEM.\hspace*{\fill}[from zero]
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \(\psi_i\) 是这个个体自己的 PK/PD 参数，比如 \(\textit{CL}_i,V_i,k_{a,i},\textit{EC}_{50,i}\)；
...
\]
\[
  B=\{\textit{AUC}_{0-24} > \textit{AUC}_{\mathrm{tox}}\},
\]
\[
  G=\{\textit{CL}_i \text{ is below the 10th population percentile}\}.
\]
...
\[
  A=\{\rv{\textit{CL}}_i < 0.6\,\textit{CL}_{\pop}\}.
\]
于是
\[
  \Prob(\rv{\textit{CL}}_i < 0.6\,\textit{CL}_{\pop}\given \rv{y}_i=y_i)
\]
...
\[
  \rv{\textit{CL}}_i,\quad \rv{V}_i,\quad \rv{\textit{AUC}}_i,\quad
  \rv{C}_{\max,i},\quad \rv{T}_{>\textit{MIC},i}.
\]
Once a probability law is assigned, these summaries inherit distributions.

例如若 \(\textit{CL}_i\) 是 log-normal:
\[
  \log \rv{\textit{CL}}_i\sim \Normal(\log \textit{CL}_{\pop},\omega_{\textit{CL}}^2),
\]
那么 \(\textit{AUC}_i=D/\rv{\textit{CL}}_i\) 也有 induced distribution。This is why parameter uncertainty automatically becomes exposure uncertainty.
...
In PopPK:
\[
  \E[\rv{\textit{CL}}_i\given y_i]
  =
  \int \textit{CL}_i\,p(\textit{CL}_i\given y_i)\,\dd \textit{CL}_i.
\]
This posterior mean is an estimator under squared-error loss. It is not necessarily the same as MAP/EBE:
\[
  \widehat{\textit{CL}}^{\textit{MAP}}_i
  =
  \argmax_{\textit{CL}_i}p(\textit{CL}_i\given y_i).
\]
...
\[
  C(\textit{AUC},\widehat{\textit{AUC}})
  =
  c_1\,1\{\textit{AUC}<\textit{AUC}_{\mathrm{eff}}\}
  +
  c_2\,1\{\textit{AUC}>\textit{AUC}_{\mathrm{tox}}\},
\]
...
\[
  \widehat{\textit{CL}}_i=4.1\,\mathrm{L/h}.
\]
But what we actually know is softer:
\[
  p(\textit{CL}_i\given y_i).
\]
If the posterior is wide or skewed, reporting only \(\widehat{\textit{CL}}_i\) hides information that may matter more than the point estimate.
...
\]
\[
  \mathcal{M}_3:\ \textit{CL}_i \text{ has weight effect}
  \quad \text{versus} \quad
  \mathcal{M}_4:\ \textit{CL}_i \text{ has weight and renal effect}.
\]
...
PopPK translation:
\[
  H_0:\ \text{one-compartment \textit{PK}},
  \qquad
  H_1:\ \text{two-compartment \textit{PK}}.
\]
or
\[
  H_0:\ \text{no renal covariate effect on }\textit{CL},
  \qquad
  H_1:\ \text{renal function modifies }\textit{CL}.
\]
...
\midrule
Posterior over \(\psi_i\) & 这个病人的 \(\textit{CL}_i,V_i\) 可能是多少？\\
...
\[
  \frac{\dd A_c(t)}{\dd t}
  =
  F k_a A_g(t) - \frac{\textit{CL}}{V}A_c(t),
  \qquad
  C(t)=\frac{A_c(t)}{V}.
\]
...
如果 \(f\) 是 linear in parameters，mixed effects theory 会比较舒服。但 PK model 通常 nonlinear: \(\textit{CL}/V\) 在指数里，\(k_a\) 和 \(\textit{CL}/V\) 可能接近，\(F\) 与 \(V\) 可能缠在一起。于是
\[
  \int p_\theta(y_i\given \psi_i)p_\theta(\psi_i)\,\dd\psi_i
\]
...
\[
  \log \rv{\textit{CL}}_i
  =
  \log \textit{CL}_{\pop} + \beta_{\textit{CL}}^{\textit{WT}}\log\!\left(\frac{\textit{WT}_i}{70}\right)
  +
  \rv{\eta}_{\textit{CL},i},
\]
\[
  \rv{\eta}_{\textit{CL},i}\sim \Normal(0,\omega_{\textit{CL}}^2).
\]
更紧凑地写：
\[
  h(\rv{\psi}_i)
...
\begin{question}
  如果加入 weight effect 后 \(\omega_{\textit{CL}}\) 下降，这是否说明 weight ``causes'' clearance variability？
\end{question}
...
假设某个 subject 只有两个 concentration samples。单独拟合 \(\textit{CL}_i,V_i,k_{a,i}\) 几乎不可能。但在 population model 下，我们可以把信息分成两部分：
\[
...
\(\widehat\psi_i^{\textit{MAP}}\) & Conditional mode for individual \(i\), often called an EBE. \\
...
A typical model:
\[
  \textit{CL}_i = \theta_{\textit{CL}}\exp(\eta_{\textit{CL},i}),
  \qquad
...
\[
  y_{ij}
  =
  f(t_{ij};\textit{CL}_i,V_i,k_{a,i})
  +
  \epsilon_{ij}.
\]
...
\[
  ...
  =
  1-
  \frac{I_{\max}C(t)}{\textit{IC}_{50}+C(t)}.
\]
...
\[
  ...
  =
  1-
  \frac{I_{\max}C_e(t)}{\textit{IC}_{50}+C_e(t)}.
\]
...
\[
  ...
  k_{\mathrm{in}}
  \left(
    1-\frac{I_{\max}C(t)}{\textit{IC}_{50}+C(t)}
  \right)
  -
  k_{\mathrm{out}}R(t).
\]
