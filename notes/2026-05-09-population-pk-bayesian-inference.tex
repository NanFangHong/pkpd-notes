...
\def\nyear {2026}
\def\nlecturer {MIT 6.008/6.437/6.435 and Marc Lavielle}
...
{\small
  ...
  These notes connect the inference language of MIT 6.008/6.437, the Bayesian modeling workflow of MIT 6.435, and Marc Lavielle's mixed effects population approach, but the final organizing object is not a course list.  It is the hierarchy
  \[
    \theta
    \longrightarrow
    \rpsi_i
    ...
    a_i,
  \]
  from population law to individual latent physiology, observed data, future prediction, and decision.  Everything below asks how that hierarchy is defined, conditioned, approximated, criticized, and used.\hspace*{\fill}[inference]

  \vspace{5pt}
  \noindent\textbf{Audience}\\
  The intended reader has not taken 6.008/6.437 and has not used NONMEM.  We begin with probability laws, conditioning, Bayes rule, random variables, and decisions, but each mathematical idea is immediately tied back to a PopPK role: observation, latent individual parameter, population distribution, predictive simulation, or dosing decision.\hspace*{\fill}[from zero]

  \vspace{5pt}
  \noindent\textbf{Style}\\
  This is a reconstruction rather than an excerpt. The notation borrows the 6.437 random-variable convention: Latin random quantities use sans-serif symbols, Greek random quantities use bold Greek, and realizations or candidate values use ordinary type. The Population PK structure follows Lavielle's \(y_i,\psi_i,\theta\) language, and the prose keeps returning to the same hierarchy so the reader can feel the connections rather than only collect formulas.\hspace*{\fill}[notation]
}
...
6.437 的一个很美的符号习惯是：random variables 用 sans-serif，比如 \(\rv{x}\)；具体实现值或 deterministic quantities 用普通 serif，比如 \(x\)。这篇笔记从一开始就采用这个区分，因为 Population PK 最容易混淆的正是 random quantity、realized data、candidate parameter value 和 deterministic population parameter。
\[
  \begin{aligned}
    \rv{y}_i &\quad \text{is random data before observation},\\
    y_i &\quad \text{is the observed dataset},\\
    \rpsi_i &\quad \text{is the latent random individual parameter},\\
    \psi_i &\quad \text{is a candidate or realized value of that parameter}.
  \end{aligned}
\]

\begin{notation}
  Latin random variables use sans-serif, e.g. \(\rv{x},\rv{y},\rv{CL},\rv{AUC}\). Greek letters are a visual exception: in standard LaTeX, \(\mathsf{\psi}\) looks almost the same as \(\psi\), so random Greek quantities use bold Greek: \(\rpsi,\reta,\rtheta,\repsilon,\rphi\). Realized or candidate Greek values remain ordinary: \(\psi,\eta,\theta,\epsilon,\phi\).
\end{notation}

\subsection{Operators, sets, labels, and PK symbols}

Probability and expectation follow the 6.437 visual convention:
\[
  \Prob(\rv{x}=x)=p_{\rv{x}}(x),
  \qquad
  \E[g(\rv{x})]=\int g(x)p_{\rv{x}}(x)\,\dd x.
\]
Sets and events use calligraphic letters, for example \(\mathcal{X}\) for an alphabet and \(\mathcal{A}\) for an event.  Pharmacometric multi-letter quantities use two roles:
\[
  \begin{aligned}
    \rv{CL}_i,\ \rv{AUC}_i
    &\quad \text{for random quantities},\\
    \pksym{CL}_i,\ \pksym{AUC}_i
    &\quad \text{for values or deterministic symbols}.
  \end{aligned}
\]
Subscripts and method names that are labels use roman type:
\[
  \omega_{\pklab{CL}}^2,
  \qquad
  \widehat\psi_i^{\pklab{MAP}},
  \qquad
  \psi_i^{\pklab{PK}}.
\]

\begin{warning}
  A point estimate such as \(\widehat\psi_i^{\pklab{MAP}}\) is not the same object as the latent random parameter \(\rpsi_i\).  The former is a decision or summary; the latter is an uncertain quantity under a probability law.  This distinction is the whole reason for the typography.
\end{warning}

\section{The spine: one hierarchy, six moves}

\subsection{One sentence}

The whole note can be compressed into one sentence:
\begin{quote}
  Population PK is a probability law in which a population parameter \(\theta\) defines a distribution over latent individual physiology \(\rpsi_i\), the individual physiology and design \(u_i\) generate observations \(\rv{y}_i\), the observations update belief about individuals and populations, and the resulting predictive law supports a pharmacological decision \(a_i\).
\end{quote}

  The long version is only a careful unfolding of that sentence.  6.008 names the probability objects, 6.437 names the inference and decision operations, 6.435 names the Bayesian modeling workflow, Lavielle names the mixed-effects hierarchy, and NONMEM/SAEM name historically important computational approximations.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{MIT 6.435}: Bayesian modeling and inference。它补上的不是另一个 estimator，而是完整 workflow: identify the analysis goal, build a prior-likelihood model, compute or approximate the posterior, summarize the posterior, simulate predictions, check the model, and revise \cite{mit6435notes}。
...
\subsection{The learning order versus the final logic}
...
\begin{warning}
  如果跳过 6.008，6.437 里的 posterior、risk、EM、typicality、model selection 会变成一串公式；如果跳过 6.437，Population PK 里的 likelihood、EBE、SAEM、posterior predictive check 又会像软件黑话；如果跳过 6.435，Bayesian 分析会被误解成一次性算 posterior，而不是模型构造、模拟、推断、检查、修正的一整圈。6.008 是语感，6.437 是深度，6.435 是 workflow，Lavielle 是落地。
\end{warning}

但成书后的逻辑不能只是课程顺序。课程顺序回答 ``我该先学什么''；笔记主线回答 ``这些东西如何变成同一个 PopPK inference system''。所以后面每个章节都会回到同一条链：
\[
  \begin{array}{c}
    \text{population law}\\
    \downarrow\\
    \text{latent individual physiology}\\
    \downarrow\\
    \text{observed clinical/pharmacological data}\\
    \downarrow\\
    \text{posterior and marginal likelihood}\\
    \downarrow\\
    \text{future prediction and decision}
  \end{array}
\]
...
Different communities describe the same hierarchy with different surface languages:
\[
  \begin{array}{lll}
    \text{6.008} &:& \text{sample space, conditioning, random variables, decision rules},\\
    \text{6.437} &:& \text{posterior, risk, marginalization, EM, information, sampling},\\
    \text{6.435} &:& \text{prior, likelihood, posterior, predictive simulation, model checking},\\
    \text{Lavielle} &:& y_i,\psi_i,\theta,c_i,u_i,\text{ tasks and methods},\\
    \text{NONMEM} &:& \pklab{THETA},\pklab{ETA},\pklab{EPSILON},\Omega,\Sigma,\pklab{OFV}.
  \end{array}
\]
The note becomes easier once we stop treating these as separate dialects and translate them back to one object:
\[
  p_\theta(\psi_i\given c_i)\,
  p_\theta(y_i\given \psi_i,u_i).
\]
This product is the quiet center of the book.  It is a probability law, a graphical model, a mixed-effects model, a computational challenge, and a clinical prediction engine all at once.

\subsection{A reader's map}

The chapters have a dependency, not merely an order:
\[
  \begin{array}{c}
    \text{define the random objects}\\
    \downarrow\\
    \text{condition from population to individual}\\
    \downarrow\\
    \text{name the decisions and losses}\\
    \downarrow\\
    \text{write the hierarchy as a graph and factorization}\\
    \downarrow\\
    \text{marginalize latent individual parameters}\\
    \downarrow\\
    \text{approximate the hard integral}\\
    \downarrow\\
    \text{simulate predictions and criticize the model}
  \end{array}
\]
If a later section feels like software detail, return to this map and ask which arrow is being computed.

\subsection{The six moves}

The first half of the note follows six moves:
\begin{center}
\begin{tabularx}{0.96\textwidth}{p{0.18\textwidth}X}
\toprule
Move & What it contributes to the hierarchy \\
\midrule
I & Define the data-generating law before estimating anything. \\
II & Condition from population variability to an individual posterior. \\
III & Turn posterior uncertainty into estimates, doses, and model choices through loss. \\
IV & Make the hierarchy visible as a graph and factorization. \\
V & Simulate future individuals and future observations from the fitted law. \\
VI & Compare imperfect probability laws by evidence, prediction, and decision value. \\
\bottomrule
\end{tabularx}
\end{center}

The second half then repeats the same object in PopPK language: structural PK forward maps, latent individual parameters, marginal likelihood, SAEM, NONMEM, full Bayes, warfarin, diagnostics, and reading protocols.

\subsection{Why Population PK is not average curve fitting}
...
\section{Move I: define the data-generating law}
...
This is the first move in the PopPK hierarchy.  Before estimating anything, we decide what kind of world could have produced \(y_i\): which quantities are random, which are fixed by design, and which biological differences are allowed to vary between individuals.
...
One latent individual & \((\rv{y}_i,\rpsi_i)\), observations plus the subject's hidden PK parameters. \\
The population sample & \((\rv{y}_{1:N},\rpsi_{1:N})\), all individuals and all latent heterogeneity. \\
Future trial or clinic & \((\tilde{\rv{y}},\tilde{\rpsi})\), prediction for new individuals. \\
\bottomrule
\end{tabularx}
\end{center}
...
\section{Move II: condition from population to individual}
...
The point is not only the formula.  Conditioning is the moment where a population statement becomes an individual statement.  The population distribution says what a subject could have been; the subject's observations say which possibilities survive.
...
\section{Quantities of interest inherit uncertainty}
...
This section is the hinge between abstract probability and pharmacological language.  A clearance, an \(\pksym{AUC}\), a target-attainment event, and a future trough are all random variables once they are functions of latent physiology or future observations.
...
\[
  \rv{y}_{ij}\indep \rv{y}_{ik}\given \rpsi_i
  \quad (j\ne k).
\]
...
\section{Move III: turn posterior uncertainty into decisions}
...
\[
  ...
  =
  \argmin_a
  \E\{C(\rpsi_i,a)\given y_i\}.
\]
Different losses give different summaries:

This is where PopPK stops being only estimation.  A posterior distribution is soft information; a dose, a model choice, or an individual parameter table is a hard action or summary.  The bridge between them is loss.
...
\section{Move IV: make the hierarchy explicit}
...
The graph is therefore not decoration.  It is the grammar that tells us what can be multiplied, what can be integrated one subject at a time, and where model criticism should look when residuals show structure.
...
\begin{tikzpicture}[>=stealth, node distance=1.55cm, every node/.style={align=center}]
  ...
  \node[draw, rounded corners, below=of theta, minimum width=1.4cm, minimum height=0.7cm] (psi) {\(\rpsi_i\)};
...
\section{Move V: simulate future subjects and future data}
...
Simulation is not an optional visualization layer.  Once the hierarchy defines \(p_\theta(\psi_i\given c_i)\) and \(p_\theta(y_i\given\psi_i,u_i)\), simulation is the natural way to ask what the model believes could happen next.

\subsection{Sampling future subjects}

A population model can simulate future individuals:
\[
  \tilde{\rpsi} \sim p_{\widehat\theta}(\psi\given \tilde c),
  \qquad
  \tilde{\rv{y}}\sim p_{\widehat\theta}(y\given \tilde\psi,\tilde u).
\]
...
\section{Move VI: compare approximating probability laws}
...
That sentence will become important again in VPC/PPC, model reduction, and clinical model use: model choice is always relative to a question.
...
\section{6.437: deepen the same hierarchy}
...
So 6.437 is not a detour.  It supplies the language for the hard parts of the hierarchy: what posterior summaries mean, why the individual integral is difficult, how algorithms approximate it, and how decisions depend on loss.
...
\section{6.435: Bayesian modeling as a workflow}

\subsection{The loop: build, infer, simulate, criticize}

6.435 adds a missing layer to the story.  6.008 and 6.437 can make Bayesian inference sound like a formula:
\[
  p(\theta\given y)\propto p(y\given\theta)p(\theta).
\]
But Bayesian modeling in practice is a loop:
\[
  \begin{array}{c}
    \text{name the analysis goal}\\
    \downarrow\\
    \text{build prior and likelihood}\\
    \downarrow\\
    \text{compute or approximate the posterior}\\
    \downarrow\\
    \text{simulate posterior predictions}\\
    \downarrow\\
    \text{check, criticize, and revise the model}.
  \end{array}
\]
This is exactly the rhythm I wanted in my earlier quantitative pharmacology note: simulation and Bayesian inference are the two directions of the same map \cite{hong2020quantpharm}.  Simulation goes from parameters to data:
\[
  \theta,\psi_i,u_i
  \longmapsto
  \tilde y_i,
\]
while Bayesian inference goes from data back to plausible parameters:
\[
  y_i
  \longmapsto
  p(\theta,\psi_i\given y_i).
\]
Good quantitative pharmacology lives by moving back and forth between these two arrows.  If the simulated world does not resemble the observed world, the inverse inference was performed under the wrong forward story.

\subsection{Prior predictive and posterior predictive thinking}

For PopPK, prior predictive simulation asks:
\[
  \rpsi_i\sim p_\theta(\psi_i\given c_i),
  \qquad
  \rv{y}_i\sim p_\theta(y_i\given \rpsi_i,u_i),
\]
before looking at the data.  This checks whether the model can even generate biologically plausible subjects, concentrations, biomarkers, or adverse events.

Posterior predictive simulation asks:
\[
  \tilde{\rv{y}}_i
  \sim
  p(\tilde y_i\given y)
  =
  \int
  p(\tilde y_i\given \psi_i,\theta)
  p(\psi_i,\theta\given y)
  \dd\psi_i\,\dd\theta.
\]
This checks whether the fitted model can regenerate the aspects of the observed data that matter for the scientific question.  VPC and PPC are therefore not decorative diagnostics.  They are the forward half of Bayesian inference returning to inspect the inverse half.

\subsection{Exchangeability and why population parameters exist}

6.435's discussion of exchangeability and de Finetti gives a clean philosophical support for Population PK.  If subjects are treated as exchangeable after conditioning on covariates, then a shared latent population law is not arbitrary decoration.  It is the object that makes individual records conditionally independent:
\[
  p(y_{1:N},\psi_{1:N}\given\theta,c_{1:N},u_{1:N})
  =
  \prod_{i=1}^N
  p_\theta(\psi_i\given c_i)
  p_\theta(y_i\given\psi_i,u_i).
\]
The moment we doubt exchangeability, we should ask what is missing: site effects, batch effects, adherence, genotype, disease subtype, time-varying physiology, or treatment history.  This is more useful than saying vaguely that the model has unexplained variability.  It tells us which plate should be split or which parent node should be added.

\subsection{Approximation is also a modeling choice}

6.435 also changes how I read approximate inference.  VB, ADVI, MCMC, importance sampling, SAEM, FOCE, and Laplace are not interchangeable buttons.  Each gives access to different functionals of a hard posterior or marginal likelihood.  So the question is not only
\[
  \text{Did the algorithm converge?}
\]
but also
\[
  \text{Did the approximation preserve the posterior feature needed for the decision?}
\]
A variational approximation may give useful posterior means but underestimate uncertainty; a Laplace approximation may work near a single mode but miss skewness or multimodality; an MCMC chain may target the right distribution but mix poorly.  In PopPK, these distinctions matter because dose decisions, covariate claims, and subgroup discovery may depend more on tails and correlations than on posterior means.

\subsection{Nonparametric Bayes and unknown heterogeneity}

The Bayesian nonparametric part of 6.435 is not needed for the basic PopPK hierarchy, but it widens the imagination.  Classical PopPK usually assumes a fixed-dimensional random-effects distribution, often Gaussian after transformation.  But real populations may contain latent subgroups: slow and fast metabolizers, mechanistic responder classes, disease endotypes, adherence phenotypes, or toxicity-susceptible states.

A mixture model writes
\[
  z_i\sim \mathrm{Categorical}(\pi),
  \qquad
  \rpsi_i\given z_i=k\sim p_{\theta_k}(\psi),
  \qquad
  \rv{y}_i\given \rpsi_i\sim p(y_i\given\psi_i).
\]
A Dirichlet-process-like view lets the number of occupied subgroups grow with data, rather than fixing it at the beginning.  The practical lesson is modest but important: sometimes ``unexplained variability'' is not merely wider \(\Omega\); it may be unresolved structure.

I would not put a nonparametric prior into every pharmacometric analysis.  But I would keep the 6.435 question alive:
\begin{quote}
  Are we forcing a simple population distribution to absorb biological heterogeneity that should be represented as latent structure?
\end{quote}

\section{Model comparison as evidence and decision}
...
\section{Posterior summaries are loss-dependent}
...
\[
  ...
  \text{ solves }
  \Prob(\rpsi_i\le \widehat{\psi}^{median}_i\given y_i)=\frac{1}{2}.
\]
...
\section{Complete-data structure and exponential-family intuition}
...
\section{Marginalization makes EM necessary}
...
\section{Sampling and typicality in high-dimensional PopPK}
...
\section{The hierarchy in its PopPK form}
...
\section{Decision theory before software output}
...
\section{Structural PK as a forward map inside probability}
...
\[
  ...
  f(t_{ij};\psi_i,u_i)
  +
  g(t_{ij};\psi_i,\xi)\repsilon_{ij},
  \qquad
  \repsilon_{ij}\iid \Normal(0,1).
\]
...
\section{Individual parameters as latent physiology}

\subsection{The Lavielle layer}

Lavielle 的 population approach 可以读成：
\[
  \rpsi_i \sim p_\theta(\psi\given c_i),
  \qquad
  \rv{y}_i\given \rpsi_i=\psi_i \sim p_\theta(y\given \psi_i,u_i).
\]
...
\]
\[
  \reta_{\pklab{CL},i}\sim \Normal(0,\omega_{\pklab{CL}}^2).
\]
更紧凑地写：
\[
  h(\rpsi_i)
...
\begin{itemize}[leftmargin=2em]
  ...
  \[
    h(\rpsi_i)-h(\widetilde\psi_i)=\reta_i.
  \]
\end{itemize}
...
\section{The full graphical model and factorization}
...
\begin{tikzpicture}[>=stealth, node distance=2.2cm, every node/.style={align=center}]
  ...
  \node[draw, rounded corners, below left=1.1cm and 1.3cm of theta, minimum width=1.7cm, minimum height=0.75cm] (psi) {\(\rpsi_i\)};
...
\section{Population, individual, and predictive targets}
...
\section{The incomplete-data problem: where computation begins}
...
\section{NONMEM as the same hierarchy in another dialect}
...
\[
  \begin{aligned}
    ...
    \repsilon_{ij} &\sim \Normal(0,\Sigma)
      \quad \text{residual errors}.
  \end{aligned}
\]
A typical model:
\[
  \pksym{CL}_i = \theta_{\pklab{CL}}\exp(\reta_{\pklab{CL},i}),
  \qquad
  V_i = \theta_V\exp(\reta_{V,i}),
\]
\[
  y_{ij}
  =
...
\section{Full Bayesian Population PK: one more layer of uncertainty}

\subsection{Adding a prior on population parameters}

Empirical Bayes PopPK estimates \(\theta\), then conditions on \(\widehat\theta\). Full Bayesian PopPK treats the population parameter as a random quantity, \(\rtheta\), by giving its value \(\theta\) a prior:
\[
  p(\theta,\psi_{1:N}\given y)
  \propto
  p(\theta)
  ...
  p_\theta(y_i\given \psi_i,u_i).
\]
Then posterior uncertainty in \(\rtheta\) is propagated into individual parameters and predictions:
\[
  p(\tilde y\given y)
  =
  \int
...
Empirical Bayes individual estimation & \(\rpsi_i\) is inferred conditionally on \(\widehat\theta\); population uncertainty is often ignored in the individual posterior. \\
Full Bayesian & \(\rtheta\) and \(\rpsi_i\) are jointly uncertain; posterior and posterior predictive intervals naturally include both levels. \\
\bottomrule
\end{tabularx}
\end{center}
...
\section{Warfarin as a running example of missing structure}
...
\section{Model evaluation as predictive criticism}
...
\section{How to read a PopPK paper as an inference system}
...
\section{Slow pass: the same hierarchy with more texture}

The main chapters above establish the spine.  The next block slows down and revisits the same hierarchy with more connective tissue.  It is not a separate essay; it is the same route at a lower altitude:
\[
  \begin{array}{c}
    \text{probability law}\\
    \downarrow\\
...
\input{population-pk-bayesian-inference-expanded}

\section{Worked notebook: the hierarchy in examples}

The worked notebook turns the abstract hierarchy into small calculable objects.  The order is deliberate: first borrowing strength, then lognormal variability and covariates, then residual error, then the hard individual integral, then SAEM/MCMC, then predictive checks and report reading.

\input{population-pk-bayesian-inference-worked}

\section{Source-guided reading route}

The final route maps the synthesis back to the original sources.  It is meant as a reading loop: read a source section, rewrite its probability law, identify the latent variables, name the approximation, and ask what prediction or decision the model supports.

\input{population-pk-bayesian-inference-source-guided}
