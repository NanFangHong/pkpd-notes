...
{\small
  ...
  The intended reader has not taken 6.008/6.437 and has not used NONMEM. We begin with probability laws, conditioning, Bayes rule, and latent variables, then move gradually into \(PK\) ODEs, individual parameters, population distributions, marginal likelihood, SAEM, and NONMEM.\hspace*{\fill}[from zero]
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \(\psi_i\) 是这个个体自己的 PK/PD 参数，比如 \(CL_i,V_i,k_{a,i},EC_{50,i}\)；
...
\]
\[
  B=\{AUC_{0-24} > AUC_{\mathrm{tox}}\},
\]
\[
  G=\{CL_i \text{ is below the 10th population percentile}\}.
\]
...
\[
  A=\{\rv{CL}_i < 0.6\,CL_{\pop}\}.
\]
于是
\[
  \Prob(\rv{CL}_i < 0.6\,CL_{\pop}\given \rv{y}_i=y_i)
\]
...
\[
  ...
  \rv{C}_{\max,i},\quad \rv{T}_{>MIC,i}.
\]
Once a probability law is assigned, these summaries inherit distributions.

例如若 \(CL_i\) 是 log-normal:
\[
  \log \rv{CL}_i\sim \Normal(\log CL_{\pop},\omega_{CL}^2),
\]
那么 \(AUC_i=D/\rv{CL}_i\) 也有 induced distribution。This is why parameter uncertainty automatically becomes exposure uncertainty.
...
\[
  C(AUC,\widehat{AUC})
  =
  c_1\,1\{AUC<AUC_{\mathrm{eff}}\}
  +
  c_2\,1\{AUC>AUC_{\mathrm{tox}}\},
\]
...
\[
  \widehat{CL}_i=4.1\,\mathrm{L/h}.
\]
But what we actually know is softer:
\[
  p(CL_i\given y_i).
\]
If the posterior is wide or skewed, reporting only \(\widehat{CL}_i\) hides information that may matter more than the point estimate.
...
\]
\[
  \mathcal{M}_3:\ CL_i \text{ has weight effect}
  \quad \text{versus} \quad
  \mathcal{M}_4:\ CL_i \text{ has weight and renal effect}.
\]
...
or
\[
  H_0:\ \text{no renal covariate effect on }CL,
  \qquad
  H_1:\ \text{renal function modifies }CL.
\]
...
\[
  \widehat{\psi}^{\mathrm{MAP}}_i
  =
  \argmax_{\psi_i}
  p_\theta(\psi_i\given y_i),
\]
\[
  \widehat{\psi}^{\mathrm{MMSE}}_i
  =
...
\midrule
Posterior over \(\psi_i\) & 这个病人的 \(CL_i,V_i\) 可能是多少？\\
...
如果 \(f\) 是 linear in parameters，mixed effects theory 会比较舒服。但 PK model 通常 nonlinear: \(CL/V\) 在指数里，\(k_a\) 和 \(CL/V\) 可能接近，\(F\) 与 \(V\) 可能缠在一起。于是
\[
  \int p_\theta(y_i\given \psi_i)p_\theta(\psi_i)\,\dd\psi_i
\]
...
\[
  \log \rv{CL}_i
  =
  \log CL_{\pop} + \beta_{CL}^{WT}\log\!\left(\frac{WT_i}{70}\right)
  +
  \rv{\eta}_{CL,i},
\]
\[
  \rv{\eta}_{CL,i}\sim \Normal(0,\omega_{CL}^2).
\]
更紧凑地写：
\[
  h(\rv{\psi}_i)
...
\begin{question}
  如果加入 weight effect 后 \(\omega_{CL}\) 下降，这是否说明 weight ``causes'' clearance variability？
\end{question}
...
假设某个 subject 只有两个 concentration samples。单独拟合 \(CL_i,V_i,k_{a,i}\) 几乎不可能。但在 population model 下，我们可以把信息分成两部分：
\[
...
\(\widehat\psi_i^{MAP}\) & Conditional mode for individual \(i\), often called an EBE. \\
...
\[
  ...
  =
  1-
  \frac{I_{\max}C_e(t)}{IC_{50}+C_e(t)}.
\]
