\subsection{A two-level normal model}

\subsubsection*{Why start with a toy model}
...
Let
\[
  \rpsi_i\mid \mu,\omega^2 \sim \Normal(\mu,\omega^2),
  \qquad
  \rv{y}_{ij}\mid \rpsi_i=\psi_i,\sigma^2\sim\Normal(\psi_i,\sigma^2),
  \qquad
  j=1,\ldots,n_i.
\]
Think of \(\rpsi_i\) as an individual log-clearance before it is known, and \(y_{ij}\) as realized noisy observations of that log-clearance after some simplification.  This is not a realistic PK model, but it contains the same hierarchy:
\[
  \text{population} \longrightarrow \text{individual parameter} \longrightarrow \text{observations}.
\]

\subsubsection*{The individual posterior}

Let \(\bar y_i=n_i^{-1}\sum_j y_{ij}\).  Conditional on \(\mu,\omega^2,\sigma^2\), the posterior for \(\rpsi_i\) is normal:
\[
  \rpsi_i\mid y_i
  \sim
  \Normal(m_i,s_i^2),
...
\subsubsection*{What this teaches about EBEs}
...
\subsubsection*{Predictive distribution for a future observation}
...
\subsection{Lognormal random effects}

\subsubsection*{Why log transforms are common}
...
\subsubsection*{Typical value versus mean}
...
\[
  \pksym{CL}_i=\pksym{CL}_{\pop}\left(\frac{\pksym{WT}_i}{70}\right)^\beta\exp(\reta_i).
\]
...
\subsubsection*{Correlation between random effects}

If clearance and volume random effects are correlated,
\[
  \begin{pmatrix}\reta_{\pklab{CL},i}\\ \reta_{V,i}\end{pmatrix}
  \sim
  \Normal\left(
...
\subsubsection*{Allometry as prior biological structure}
...
\subsubsection*{Covariate selection as model comparison}
...
\subsubsection*{Additive, proportional, combined}
...
\subsubsection*{Lognormal residual error}
...
\subsubsection*{BLQ as censoring}
...
\subsection{Linearization and Laplace}

\subsubsection*{The hard integral again}
...
\subsubsection*{FO and FOCE intuition}
...
\subsection{SAEM in slow motion}

\subsubsection*{Missing data view}
...
\subsubsection*{Why the step size matters}
...
\subsubsection*{The Bayesian-looking part}
...
\subsubsection*{Comparison with MCMC full Bayes}
...
\subsection{Metropolis-Hastings for one subject}
...
\subsubsection*{Why tuning is inferential}
...
\subsection{Posterior predictive checks and VPC}

\subsubsection*{The simulation recipe}
...
\subsubsection*{Prediction intervals are conditional on design}
...
\subsection{Information geometry intuition}

\subsubsection*{Models as approximating families}
...
\subsubsection*{Why misspecification matters}
...
\subsubsection*{Projection language for model building}
...
\subsection{From individual to population}

\subsubsection*{The individual approach}
...
\subsubsection*{The population approach}
...
\subsubsection*{When the individual approach is still useful}
...
\subsection{Model reports}

\subsubsection*{What I want from a report}
...
\subsubsection*{Parameter table reading}
...
\subsubsection*{Diagnostics reading}
