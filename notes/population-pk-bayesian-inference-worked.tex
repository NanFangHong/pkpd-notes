...
Many PK parameters are positive: clearance, volume, rate constants, bioavailability fractions after suitable transformations.  A normal model directly on \(\textit{CL}_i\) can assign probability to negative clearance.  A lognormal model avoids this:
\[
  \log \textit{CL}_i=\log \textit{CL}_{\pop}+\eta_{\textit{CL},i},
  \qquad
  \eta_{\textit{CL},i}\sim\Normal(0,\omega_{\textit{CL}}^2).
\]
Then \(\textit{CL}_i>0\) automatically.  More importantly, variability becomes multiplicative:
\[
  \textit{CL}_i=\textit{CL}_{\pop}\exp(\eta_{\textit{CL},i}).
\]
If \(\eta_{\textit{CL},i}=0.2\), clearance is multiplied by \(\exp(0.2)\).  If \(\eta_{\textit{CL},i}=-0.2\), it is multiplied by \(\exp(-0.2)\).  The model is symmetric on the log scale, not the original scale.

\subsection{Typical value versus mean}

For a lognormal parameter,
\[
  \E(\textit{CL}_i)=\textit{CL}_{\pop}\exp(\omega_{\textit{CL}}^2/2).
\]
Thus \(\textit{CL}_{\pop}\) is the median and typical value on the original scale, not the arithmetic mean.  This distinction matters when interpreting fixed effects.  A report that calls \(\textit{CL}_{\pop}\) the mean clearance is using language loosely unless \(\omega^2\) is small.

The same issue appears with covariates:
\[
  \textit{CL}_i=\textit{CL}_{\pop}\left(\frac{\textit{WT}_i}{70}\right)^\beta\exp(\eta_i).
\]
The covariate function gives the typical clearance for weight \(\textit{WT}_i\).  The population mean at that weight is larger by the lognormal factor if the random effect variance is nonzero.

\subsection{Correlation between random effects}

If clearance and volume random effects are correlated,
\[
  \begin{pmatrix}\eta_{\textit{CL},i}\\ \eta_{V,i}\end{pmatrix}
  \sim
  \Normal\left(
    \begin{pmatrix}0\\0\end{pmatrix},
    \begin{pmatrix}
      \omega_{\textit{CL}}^2 & \rho\omega_{\textit{CL}}\omega_V\\
      \rho\omega_{\textit{CL}}\omega_V & \omega_V^2
    \end{pmatrix}
  \right).
\]
...
\[
  t_{1/2,i}=\frac{\log 2\,V_i}{\textit{CL}_i}.
\]
If \(V_i\) and \(\textit{CL}_i\) rise together, half-life may be less variable than either parameter alone.  A diagonal \(\Omega\) can therefore distort derived quantities even when marginal variability looks plausible.
...
\[
  \textit{CL}_i=\textit{CL}_{\pop}\left(\frac{\textit{WT}_i}{70}\right)^\beta\exp(\eta_i).
\]
Then \(\textit{CL}_{\pop}\) is the typical clearance at \(70\,\mathrm{kg}\).  Without centering, the intercept may refer to an impossible subject, making interpretation and numerical estimation worse.

Centering also helps separate baseline physiology from covariate slope.  If age is centered at \(50\) years,
\[
  \log \textit{CL}_i=\log \textit{CL}_{\pop}+\beta_{age}(\textit{AGE}_i-50)+\eta_i,
\]
then \(\textit{CL}_{\pop}\) is typical clearance at age 50.  The slope describes multiplicative change per year on the log scale.
...
\[
  \textit{CL}_i=\textit{CL}_{\pop}\left(\frac{\textit{WT}_i}{70}\right)^{0.75}\exp(\eta_{\textit{CL},i}),
  \qquad
  V_i=V_{\pop}\left(\frac{\textit{WT}_i}{70}\right)\exp(\eta_{V,i}).
\]
...
\[
  \textit{CL}_i=\textit{CL}_{\pop}\exp(\beta z_i)\exp(\eta_i).
\]
...
\[
  P(\tilde C_{\max}>C_{\mathrm{tox}}),
  \qquad
  P(\tilde C_{\min}<C_{\mathrm{eff}}),
  \qquad
  P(\widetilde{\textit{AUC}}>U).
\]
...
\[
  ...
  \textit{VPC} & \text{Is the population predictive distribution credible?}\\
