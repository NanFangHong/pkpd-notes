...
Many PK parameters are positive: clearance, volume, rate constants, bioavailability fractions after suitable transformations.  A normal model directly on \(CL_i\) can assign probability to negative clearance.  A lognormal model avoids this:
\[
  \log CL_i=\log CL_{\pop}+\eta_{CL,i},
  \qquad
  \eta_{CL,i}\sim\Normal(0,\omega_{CL}^2).
\]
Then \(CL_i>0\) automatically.  More importantly, variability becomes multiplicative:
\[
  CL_i=CL_{\pop}\exp(\eta_{CL,i}).
\]
If \(\eta_{CL,i}=0.2\), clearance is multiplied by \(\exp(0.2)\).  If \(\eta_{CL,i}=-0.2\), it is multiplied by \(\exp(-0.2)\).  The model is symmetric on the log scale, not the original scale.

\subsection{Typical value versus mean}

For a lognormal parameter,
\[
  \E(CL_i)=CL_{\pop}\exp(\omega_{CL}^2/2).
\]
Thus \(CL_{\pop}\) is the median and typical value on the original scale, not the arithmetic mean.  This distinction matters when interpreting fixed effects.  A report that calls \(CL_{\pop}\) the mean clearance is using language loosely unless \(\omega^2\) is small.

The same issue appears with covariates:
\[
  CL_i=CL_{\pop}\left(\frac{WT_i}{70}\right)^\beta\exp(\eta_i).
\]
The covariate function gives the typical clearance for weight \(WT_i\).  The population mean at that weight is larger by the lognormal factor if the random effect variance is nonzero.

\subsection{Correlation between random effects}

If clearance and volume random effects are correlated,
\[
  \begin{pmatrix}\eta_{CL,i}\\ \eta_{V,i}\end{pmatrix}
  \sim
  \Normal\left(
    \begin{pmatrix}0\\0\end{pmatrix},
    \begin{pmatrix}
      \omega_{CL}^2 & \rho\omega_{CL}\omega_V\\
      \rho\omega_{CL}\omega_V & \omega_V^2
    \end{pmatrix}
  \right).
\]
...
\[
  t_{1/2,i}=\frac{\log 2\,V_i}{CL_i}.
\]
If \(V_i\) and \(CL_i\) rise together, half-life may be less variable than either parameter alone.  A diagonal \(\Omega\) can therefore distort derived quantities even when marginal variability looks plausible.
...
\[
  CL_i=CL_{\pop}\left(\frac{WT_i}{70}\right)^\beta\exp(\eta_i).
\]
Then \(CL_{\pop}\) is the typical clearance at \(70\,\mathrm{kg}\).  Without centering, the intercept may refer to an impossible subject, making interpretation and numerical estimation worse.

Centering also helps separate baseline physiology from covariate slope.  If age is centered at \(50\) years,
\[
  \log CL_i=\log CL_{\pop}+\beta_{age}(AGE_i-50)+\eta_i,
\]
then \(CL_{\pop}\) is typical clearance at age 50.  The slope describes multiplicative change per year on the log scale.
...
\[
  CL_i=CL_{\pop}\left(\frac{WT_i}{70}\right)^{0.75}\exp(\eta_{CL,i}),
  \qquad
  V_i=V_{\pop}\left(\frac{WT_i}{70}\right)\exp(\eta_{V,i}).
\]
...
\[
  CL_i=CL_{\pop}\exp(\beta z_i)\exp(\eta_i).
\]
...
\[
  ...
  \text{VPC} & \text{Is the population predictive distribution credible?}\\
