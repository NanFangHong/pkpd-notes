...
\[
  ...
  \text{NONMEM} & \text{asks} & \text{How does this object appear in a historical pharmacometric syntax?}
  \end{array}
\]
...
\[
  ...
  \qquad
  A_3=\{\mathrm{AUC}_{0,\tau}\in[L,U]\}.
\]
These are not decorative.  They are what dosing decisions are often about.  A model that estimates \(CL_i\) well but cannot answer \(P(A_2\mid y_i)\) may be less useful than a model with slightly rougher parameter estimates but better predictive calibration in the clinically relevant tail.
...
\[
  y_{ij}=f(t_{ij};CL_i,V_i,u_i)+\varepsilon_{ij},
  \qquad
...
\[
  \log y_{ij}=\log f(t_{ij};CL_i,V_i,u_i)+\varepsilon_{ij},
  \qquad
  \varepsilon_{ij}\sim\Normal(0,\sigma^2),
\]
...
\[
  \mathrm{AUC}_i=\frac{D_i}{CL_i},
  \qquad
  t_{1/2,i}=\frac{\log 2}{k_i},
  \qquad
  C_{\max,i}=\max_t C_i(t).
\]
Once \(CL_i\), \(k_i\), or the whole curve \(C_i(t)\) is uncertain, these summaries are uncertain too.  The habit is therefore:
\[
  \text{do not only report a transformed point estimate; transform the uncertainty.}
\]
The posterior distribution of \(CL_i\) induces a posterior distribution of \(\mathrm{AUC}_i\).  The posterior predictive distribution of future concentrations induces probabilities of clinical target attainment.  This is where the random-variable language becomes directly useful.

\subsection{Expectations are summaries, not reality}

An expectation is often useful:
\[
  \E(\mathrm{AUC}_i\mid y_i)
  =
  \int \frac{D_i}{CL_i}p(CL_i\mid y_i)\,\dd CL_i.
\]
But the expectation is not the same as the whole posterior distribution.  In asymmetric or safety-critical problems, the upper tail may matter more than the mean.  A dosing rule based on \(\E(\mathrm{AUC}_i\mid y_i)\) may be inferior to a rule based on
\[
  P(\mathrm{AUC}_i>U\mid y_i)
\]
...
\begin{itemize}[leftmargin=2em]
  ...
  \item The \term{representation} is the mathematical probability law: \(CL_i=CL_{\pop}(WT_i/70)^\beta\exp(\eta_{CL,i})\), \(\eta_i\sim\Normal(0,\Omega)\), and an observation density.
...
\[
  CL_i=CL_{\pop}(WT_i/70)^{\beta}\exp(\eta_i).
\]
...
\[
  \psi_i^{PK}\longrightarrow C_i(t),
  \qquad
  \psi_i^{PD}\longrightarrow R_i(t),
  \qquad
...
\[
  \theta \leftrightarrow \text{THETA},
  \qquad
  \eta_i \leftrightarrow \text{ETA},
  \qquad
  \varepsilon_{ij} \leftrightarrow \text{EPSILON},
\]
\[
  \Omega \leftrightarrow \text{OMEGA},
  \qquad
  \Sigma \leftrightarrow \text{SIGMA}.
\]
A line such as
\[
  CL_i=TVCL\exp(\eta_{CL,i})
\]
is not just code.  It is a distributional statement:
\[
  \log CL_i\sim\Normal(\log TVCL,\omega_{CL}^2).
\]
...
\[
  \frac{\dd R(t)}{\dd t}
  =
  k_{\mathrm{in}}\left(1-\frac{I_{\max}C(t)}{IC_{50}+C(t)}\right)
  -
  k_{\mathrm{out}}R(t).
\]
