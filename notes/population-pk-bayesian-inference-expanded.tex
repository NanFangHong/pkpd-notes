\subsection{Orientation: why the slow pass exists}

\subsubsection*{What this expansion is trying to do}
...
\subsubsection*{A source hierarchy rather than a literature review}

This is not a literature review.  A literature review would place many papers side by side and compare claims.  Here I am building a reading hierarchy:
\begin{center}
\begin{tabularx}{0.96\textwidth}{p{0.18\textwidth}X}
\toprule
Source & Question it asks \\
\midrule
6.008 & What probability model are we even talking about? \\
6.437 & What inference, decision, or approximation follows from that model? \\
6.435 & How do simulation, posterior inference, predictive checking, and model revision form one workflow? \\
Lavielle & How is this hierarchy represented, estimated, diagnosed, and used? \\
\(\pklab{NONMEM}\) & How does this object appear in a historical pharmacometric syntax? \\
\bottomrule
\end{tabularx}
\end{center}
...
\subsection{6.008 carefully: probability law before Bayes rule}

\subsubsection*{The sample point is not a blood concentration}
...
\[
  (y_i,\psi_i),
\]
where \(\psi_i\), as a candidate value of \(\rpsi_i\), might contain clearance, volume, absorption rate, baseline response, turnover parameters, or sensitivity parameters.  If the design itself is randomized, the outcome may also include \(u_i\).  If dropout is informative, the event time of dropout must become part of the outcome.  The modeler is already making a scientific claim when deciding what belongs in the sample space.
...
\subsubsection*{Events are clinical subsets}
...
\[
  ...
  \qquad
  A_3=\{\pksym{AUC}_{0,\tau}\in[L,U]\}.
\]
These are not decorative.  They are what dosing decisions are often about.  A model that estimates \(\pksym{CL}_i\) well but cannot answer \(\Prob(A_2\mid y_i)\) may be less useful than a model with slightly rougher parameter estimates but better predictive calibration in the clinically relevant tail.
...
\[
  \log y_{ij}=\log f(t_{ij};\pksym{CL}_i,V_i,u_i)+\varepsilon_{ij},
  \qquad
  \varepsilon_{ij}\sim\Normal(0,\sigma^2),
\]
...
\subsubsection*{The law of total probability is the population idea}
...
\subsubsection*{Bayes rule as individual reweighting}
...
\subsection{Random variables as pharmacological summaries}

\subsubsection*{A random variable is a question asked of the outcome}
...
\[
  \pksym{AUC}_i=\frac{D_i}{\pksym{CL}_i},
  \qquad
  t_{1/2,i}=\frac{\log 2}{k_i},
  \qquad
  C_{\max,i}=\max_t C_i(t).
\]
Once \(\pksym{CL}_i\), \(k_i\), or the whole curve \(C_i(t)\) is uncertain, these summaries are uncertain too.  The habit is therefore:
\[
  \text{do not only report a transformed point estimate; transform the uncertainty.}
\]
The posterior distribution of \(\pksym{CL}_i\) induces a posterior distribution of \(\pksym{AUC}_i\).  The posterior predictive distribution of future concentrations induces probabilities of clinical target attainment.  This is where the random-variable language becomes directly useful.

\subsubsection*{Expectations are summaries, not reality}

An expectation is often useful:
\[
  \E(\pksym{AUC}_i\mid y_i)
  =
  \int \frac{D_i}{\pksym{CL}_i}p(\pksym{CL}_i\mid y_i)\,\dd \pksym{CL}_i.
\]
But the expectation is not the same as the whole posterior distribution.  In asymmetric or safety-critical problems, the upper tail may matter more than the mean.  A dosing rule based on \(\E(\pksym{AUC}_i\mid y_i)\) may be inferior to a rule based on
\[
  \Prob(\pksym{AUC}_i>U\mid y_i)
\]
if toxicity risk is the central concern.  This is where 6.008 expectation leads naturally into 6.437 decision theory.

\subsubsection*{Independence is a modeling assumption}

It is common to write
\[
  \rv{y}_{ij}\indep \rv{y}_{ik}\mid \rpsi_i,\theta
\]
for two observations from the same subject.  This does not mean the observations are marginally independent.  They are connected through the same latent individual parameter \(\rpsi_i\).  Conditional independence is a statement about what remains after we condition on the relevant latent structure.

This distinction is crucial for repeated measures.  If a model treats observations as independent without individual random effects, it may underestimate uncertainty because it ignores within-subject correlation.  If a model includes \(\rpsi_i\), much of the dependence is explained by shared physiology.  Remaining residual dependence may still exist through assay drift, disease dynamics, adherence, or unmodeled feedback.  A good PopPK reader always asks: independent after conditioning on what?

\subsection{Decision theory before dosing software}

\subsubsection*{Estimation is a decision with a loss}
...
\subsubsection*{Why clinical asymmetry matters}
...
\subsubsection*{The danger of decision-blind model selection}
...
\subsection{Graphical models for the population approach}

\subsubsection*{The basic plate}

A minimal population PK graphical model can be written as
\[
  \theta \longrightarrow \rpsi_i \longrightarrow \rv{y}_i,
  \qquad
  c_i \longrightarrow \rpsi_i,
  \qquad
  u_i \longrightarrow \rv{y}_i,
  \qquad
  i=1,\ldots,N.
\]
Here \(\theta\) contains population-level parameters, \(c_i\) covariates, \(u_i\) the design or dosing regimen, \(\rpsi_i\) individual parameters, and \(\rv{y}_i\) observations.  The factorization is
\[
...
\subsubsection*{What the plate hides}
...
\subsubsection*{The graph as a reading tool}

When reading a paper, redraw the graph before judging the model.  If the paper says ``renal function was a significant covariate on clearance,'' draw \(c_i\to \pksym{CL}_i\).  If the paper says ``inter-occasion variability was included on bioavailability,'' draw an occasion-level random effect below the subject-level random effect.  If the paper says ``BLQ data were handled with M3,'' draw the censoring event as part of the observation mechanism.
...
\subsection{Lavielle's joint-distribution thesis}

\subsubsection*{A model is not only a structural equation}
...
\[
  ...
  p_\theta(\psi_i\mid c_i).
\]
If the population parameter is modeled as a random quantity, \(\rtheta\), a Bayesian extension adds a prior density \(p(\theta)\).  If covariates or dosing records are modeled as random, they also enter.  But even the basic two-layer expression already changes how we think.  The question is not merely whether \(f(t;\psi_i)\) fits the observed curve.  The question is whether the joint law explains individual profiles, between-subject variability, residual noise, and prediction targets.

\subsubsection*{Description, representation, implementation}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item The \term{representation} is the mathematical probability law: \(\pksym{CL}_i=\pksym{CL}_{\pop}(\pksym{WT}_i/70)^\beta\exp(\reta_{\pklab{CL},i})\), \(\reta_i\sim\Normal(0,\Omega)\), and an observation density.
...
\subsubsection*{Count, categorical, and time-to-event observations}
...
\subsubsection*{Residuals are derived objects}
...
\subsection{Individual parameter models}

\subsubsection*{The population distribution}
...
\[
  ...
  =
  \log \pksym{CL}_{\pop}
  +\beta_{\pklab{WT}}\log(\pksym{WT}_i/70)
  +\reta_{\pklab{CL},i},
  \qquad
...
\subsubsection*{Covariates are not causal by default}

A covariate model is often written as if it explains biology:
\[
  \pksym{CL}_i=\pksym{CL}_{\pop}(\pksym{WT}_i/70)^{\beta}\exp(\eta_i).
\]
...
\subsubsection*{Inter-occasion variability}
...
\subsection{Structural PK as a forward map}

\subsubsection*{The model as a deterministic solver inside probability}
...
\[
  C_i(t)=\frac{D_i}{V_i}\exp\left(-\frac{\pksym{CL}_i}{V_i}t\right).
\]
...
This separation is useful.  The PK equation by itself is not a statistical model.  The statistical model begins when we say how \(\rpsi_i\) varies and how \(\rv{y}_i\) deviates from \(f\).  The forward map provides pharmacological structure; probability wraps uncertainty around it.

\subsubsection*{Identifiability and design}
...
\subsection{Marginal likelihood and the hard integral}

\subsubsection*{The individual contribution}
...
\subsubsection*{Why approximation enters}
...
\subsubsection*{The individual mode is not the integral}
...
\subsection{EM, SAEM, and missing individual parameters}
...
\subsubsection*{SAEM's pharmacometric value}
...
\begin{quote}
  SAEM is ML for population parameters using Bayesian-looking draws for individuals.
\end{quote}
...
\subsection{NONMEM as a notation and approximation culture}

\subsubsection*{THETA, ETA, EPSILON}

NONMEM's notation can be translated:
\[
  \theta \leftrightarrow \pklab{THETA},
  \qquad
  \eta_i \leftrightarrow \pklab{ETA},
  \qquad
  \varepsilon_{ij} \leftrightarrow \pklab{EPSILON},
\]
\[
  \Omega \leftrightarrow \pklab{OMEGA},
  \qquad
  \Sigma \leftrightarrow \pklab{SIGMA}.
\]
A line such as
\[
  \pksym{CL}_i=\pksym{TVCL}\exp(\eta_{\pklab{CL},i})
\]
is not just code.  It is a distributional statement:
\[
  \log \pksym{CL}_i\sim\Normal(\log \pksym{TVCL},\omega_{\pklab{CL}}^2).
\]
This translation makes NONMEM less mysterious.  It is a compact language for hierarchical probability models.

\subsubsection*{FO, FOCE, Laplace}
...
\subsubsection*{EBEs and shrinkage}
...
\subsection{Model evaluation as calibrated imagination}

\subsubsection*{Posterior predictive thinking}
...
\[
  \tilde{\rv{y}}\sim p(\tilde y\mid y)
\]
or, in an ML PopPK workflow, simulate from the fitted population model
\[
  \tilde{\rpsi}_i\sim p_{\widehat\theta}(\psi_i\mid c_i),
  \qquad
  \tilde{\rv{y}}_i\sim p_{\widehat\theta}(y_i\mid \tilde\psi_i,u_i).
\]
...
\subsubsection*{Visual predictive checks}
...
\subsubsection*{Model selection as multiple evidence streams}
...
\begin{quote}
  Choose the model whose probability law is adequate for the scientific and clinical task.
\end{quote}

\subsection{Warfarin re-read}

\subsubsection*{Why the example is instructive}
...
\subsubsection*{Effect compartment as latent state}
...
\subsubsection*{Indirect response as mechanism}

An indirect response model goes further by modeling production and loss:
\[
  \frac{\dd R(t)}{\dd t}
  =
  k_{\mathrm{in}}\left(1-\frac{I_{\max}C(t)}{\pksym{IC}_{50}+C(t)}\right)
  -
  k_{\mathrm{out}}R(t).
\]
...
\subsection{Full Bayesian Population PK}

\subsubsection*{What changes when \(\rtheta\) is random}

Classical PopPK often estimates \(\theta\) by maximum likelihood and then conditions on \(\widehat\theta\).  Full Bayesian PopPK treats \(\rtheta\) as uncertain by putting a prior on the value \(\theta\):
\[
...
\subsubsection*{Why full Bayes is not automatically superior}
...
\subsubsection*{A practical compromise}
...
\subsection{A reading protocol for PopPK papers}

\subsubsection*{Reconstruct the hierarchy}
...
\subsubsection*{Translate the software}
...
\subsubsection*{Ask the decision question}
