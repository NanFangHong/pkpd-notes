...
\[
  \text{\textit{FOCE}? Laplace? \textit{SAEM}? importance sampling? full Bayesian \textit{MCMC}?}
\]
...
\[
  \text{\textit{PK} supplies the mechanistic map;}
  \qquad
...
\[
  y_i,\quad \psi_i,\quad \theta,\quad c_i,\quad u_i.
\]
...
The target might be \(\theta\), \(\psi_i\), a covariate effect, a model comparison, a future exposure, a dose, or a clinical event probability.  Different targets require different summaries.  Do not let the software's default output decide the scientific target.
